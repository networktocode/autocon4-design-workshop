...
12\. We deployed a single node with no neighbors and no config. Could be useful for practicing CLI commands or testing some automation workflows, but not much else.


13\. Lets modify our `ac4.clab.yml` file to include a second node and a link between the two nodes
```yaml
# ac4.clab.yml
---
name: ac4
...
14\. Make sure to save once you're done!

![](https://ajeuwbhvhr.cloudimg.io/https://colony-recorder.s3.amazonaws.com/files/2025-05-24/c9963843-25dc-41f7-9315-54817c57facf/ascreenshot.jpeg?tl_px=0,0&br_px=1800,1006&force_format=jpeg&q=100&width=1120.0)


15\. In your terminal, type `clab deploy` and press Enter. After a few seconds, the new lab will be up and running.
...
16\. Feel free to SSH into the two devices and see what has changed. Maybe take a look at your interfaces and LLDP neighbors again.


17\. Now, from here, we could manually apply configurations to the nodes and run some cli commands to validate their operational state, but this is an automation workshop so lets use some automation to do that for us instead.


18\. Go ahead and exit the SSH session and destroy the topology by typing `clab destroy`
