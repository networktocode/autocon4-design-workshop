...
19\. Click the blue "Run Job Now" button
...
21\. To make the design idempotent, let use the `!create_or_update` action tag when setting the Manufacturer's name
...
22\. Save your changes and go to the "Source Control" tab in the left navbar in VSCode
...
23\. Click the plus sign (+) next to the file "0000_simpledesign.yaml.j2" to stage your changes. Type a commit message (ie `Updated for Idempotency`) and then click on the "Commit" button
...
25\. Navigate back to Nautobot in your browser, click "Extensibility"
...
27\. Click the blue sync icon to sync the repo changes into Nautobot
...
29\. Then click "Designs" in the sub menu
...
30\. Click the play button next to "Simple Design" to run the Job.
...
33\. Click the blue "Run Job Now" button
...
34\. As you can see in the logs, the design was successfully deployed in an idempotent manner
...
35\. Let's expand on our design. What about a platform for our manufacturer? You may see here that, using the dunder (double underscore), we can query for related objects
...
36\. Before committing our changes, let's see what Jinja2 can do for us here. Imagine that we want three sites in Athens, under Region Greece. Sure, we could add them one by one but what if there were 1,000 sites? Jinja allows us to use For loops for efficency.
...
38\. Click the plus sign (+) next to the file "0000_simpledesign.yaml.j2" to stage your changes. Type a commit message (ie "Added Platform and locations") and then click on the "Commit" button.
...
40\. Let's sync these latest changes into Nautobot. Navigate to Nautobot in your browser and click "Extensibility"
...
42\. Click the sync button for our Design Workshop Repo.
...
44\. Then click "Designs" in the sub menu
...
51\. Along with the location Greece, we also have the 3 sites (`ATH01`, `ATH02`, and `ATH03`)  that we defined in our Jinja2 for loop!
